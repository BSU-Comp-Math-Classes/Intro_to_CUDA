...
__global__ void diffOnGPU_shared(float *u, const int N, float dx, float *dudx )
{
 ...
 __shared__ float u_shared[THREADS_PER_BLOCK]; //declare shared array
 ...
 //operate on shared array instead
  ...
  else if(i_glb==N-1)   //at the global right end-point compute one-sided difference
  {
	...
	/*      }else if (i_loc==0){
	dudx[i_glb] = (u_shared[i_loc+1] - u[i_glb-1])/dx/2.0;
      }else if (i_loc==THREADS_PER_BLOCK-1){
	dudx[i_glb] = (u[i_glb+1] - u_shared[i_loc-1])/dx/2.0;
	}*/
    }
...
__global__ void diffOnGPU_shared1(float *u, const int N, float dx, float *dudx )
{
  ...
  __shared__ float u_shared[THREADS_PER_BLOCK+2]; //declare shared array including ghost points
 int i_glb = blockIdx.x * blockDim.x + threadIdx.x; 
...
// set up device
  ...
  double istart, iElaps, istarth, iElapsh, iElap_copy, iElaps_shared, iElaps_shared1; 
...
// check kernel error
  ...
  // check device results
    checkResult(hostRef, gpuRef, N);
     ...
     //time the shared memory kernel 1
 istart = seconds();
  diffOnGPU_shared1<<<grid, block>>>(d_u,N,dx,d_dudx);
 CHECK(cudaDeviceSynchronize());
 iElaps_shared1 = seconds() - istart;
...
// check kernel error
  ...
  // check device results
    ...
    printf("N = %d, threads = %d, blocks = %d,  timelapsed host = %f device_global = %f device_local = %f device_local1 = %f copy = %f\n ",N,block.x,grid.x,iElapsh,iElaps,iElaps_shared,iElaps_shared1,iElap_copy);
